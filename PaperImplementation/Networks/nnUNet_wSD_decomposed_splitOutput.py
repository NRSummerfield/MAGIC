...
if __name__ != '__main__':
    ...
    from .JointResBlock import SplitUnetResBlock, SplitUnetOutBlock, SplitUpSample, SplitUnetUpBlock
...
# things that need to be implements
# 1) [done] A res block that can handle the different branches
# 2) [done] An output block that can handle the different branches
# 3) [done] DeepUp that can handle the appropriate number of outputs to scale to
#   3a) [done] DeepUp requires a split UpSampling 
# 4) [done] A flexable architecture that can organize the break point
# 5) [done] Ensure all final outputs have all classes concatinated along the channel (dim=1) for congruity
...
class DeepUpModule(nn.Module):
    ...
    def __init__(self, 
        ...
        spatial_dims: int,
        in_channels: Sequence[int],
        out_channels: Sequence[Union[int, Sequence[int]]],
        ):
        super().__init__()

        # Convolutions to take it from N channels to out_channels
        conv_blocks = []
        if isinstance(out_channels, int): out_channels = [out_channels for _ in in_channels]

        # If the out channel is a single integer, treat it as a normal output
        # Otherwise, it is a Sequence of integers and must be treated as a split output
        for _in_channels, _out_channels in zip(in_channels, out_channels):
            if isinstance(_out_channels, int):
                conv_blocks.append(UnetOutBlock(spatial_dims=spatial_dims, in_channels=_in_channels, out_channels=_out_channels))
            else:
                conv_blocks.append(SplitUnetOutBlock(spatial_dims=spatial_dims, in_channels=_in_channels, out_channels=_out_channels))

        # Upsamples to take it from c x n x n to c x m x m
        upsamples = []
        for scale, out_channel in zip(up_scales, out_channels):
            if isinstance(out_channel, int):
                upsamples.append(UpSample(spatial_dims=spatial_dims, in_channels=out_channel, out_channels=out_channel, scale_factor=scale, mode=UpsampleMode.DECONV, interp_mode=InterpolateMode.LINEAR, bias=True, apply_pad_pool=True))
            else:
                upsamples.append(SplitUpSample(spatial_dims=spatial_dims, in_channels=out_channel, out_channels=out_channel, scale_factor=scale, bias=True))
    ...
    def forward(self, inputs: Sequence[torch.Tensor]):
        ...
        for input, conv_block, upsample in zip(inputs, self.conv_blocks, self.upsamples):
            ...
            # if output is NOT a Tensor, then it will be a Sequence and needs to be concatinated along the channel for simplicity
...
class DynDecoder(nn.Module):
    def __init__(self,
        ...
        """
        Out groups: the number of outputs per "group". Sum of outgroups must match out_channels
        split_level: default = -1 (output), descending level of splitting. i.e. split_level = 0 means the last decoder (before the output) is split
        """
        super().__init__()
    ...
    def get_bottleneck(self):
        print(self.sl, len(self.filters) - 1, self.sl == len(self.filters) - 1)
        if self.sl == len(self.filters) - 1:
            return SplitUnetResBlock(
                spatial_dims=self.spatial_dims,
                in_channels=self.filters[-2],
                out_channels=[self.filters[-1] for _ in self.out_groups],
    ...
    def get_output_block(self):
        return SplitUnetOutBlock(
            spatial_dims=self.spatial_dims, 
            in_channels=self.filters[0], 
            out_channels=self.out_groups, 
            dropout=self.dropout)
    ...
    def get_upsamples(self):
        ...
        args = dict(spatial_dims = self.spatial_dims, norm_name = self.norm_name, act_name = self.act_name, dropout = self.dropout, trans_bias = False,)
        ...
        modules = []
        sl = self.sl if self.sl != len(self.filters) - 1 else self.sl - 1
        print(sl, self.sl)
        for i in range(num_blocks):
            iter_args = dict(in_channels=in_channels[i], kernel_size=kernel_sizes[i], stride=strides[i], upsample_kernel_size=upsample_kernel_size[i])
            # (num_blocks := 6) - (self.sl := -1) - 1 = 6: i in [0, ..., 5], always less than
            # (num_blokcs := 6) - (self.sl := 0) - 1 = 5: i in [0, ..., 5], i not less than on the last one 
            if i < num_blocks - sl - 1:
                conv_block = UnetUpBlock
                iter_args['out_channels']=out_channels[i]
            else:
                conv_block = SplitUnetUpBlock
                iter_args['out_channels']=[out_channels[i] for _ in self.out_groups]
    
            modules.append(conv_block(**(args | iter_args)))
        
        return nn.ModuleList(modules)
    
    def forward(self, x: Union[Sequence[dict], dict]) -> dict[str, Union[torch.Tensor, Sequence[torch.Tensor]]]:
        # if multiple inputs are given, average / concat them and continue
        ...
        # print()
        # if isinstance(out, torch.Tensor): print(out.shape)
        # else: 
        #     for ten in out: print(ten.shape)
        # print('-'*64)
        # print()
        ...
        # If the output is not a Tensor, then it must be a Sequence and needs to be channel concatinated
        if not isinstance(out, torch.Tensor):
            out = torch.concat(out, dim=1)
...
class DynDecoder_wSD(nn.Module):
    def __init__(self,
        ...
        self.decoder = DynDecoder(spatial_dims=spatial_dims, in_channels=in_channels, out_channels=out_channels, out_groups=out_groups, filters=filters, kernel_size=kernel_size, strides=strides, norm_name=norm_name, act_name=act_name, dropout=dropout, join_mode=join_mode, split_level=split_level)

        _out_channels = [out_channels for _ in scale_factors]
        sl = split_level if split_level != len(filters) - 1 else split_level - 1
        if sl >= 0:
            for i in range(sl + 1):
                _out_channels[~i] = out_groups
        self.deep_up = DeepUpModule(up_scales=scale_factors[::-1], spatial_dims=spatial_dims, in_channels=filters[:-1][::-1], out_channels=_out_channels)
...
if __name__ == '__main__':
    ...
    model_params['split_level'] = 5
    model_params['out_groups'] = [2, 10, 9, 3]
