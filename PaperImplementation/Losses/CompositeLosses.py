...
class CompositeLoss(_Loss, losses.Loss):
    ...
    def __init__(self, 
                 ...
                 encoder_keys_for_self_distillation: list[str] = ['encoder'],
                 ):
        ...
        ], include_background=True, T=temperature, encoder_keys=encoder_keys_for_self_distillation)
        self.loss_fn = {
...
class MultiGrpLoss(_Loss):
    def __init__(self, 
                 grp_idxs: list[list[int]],
                 loss_fns: list[Any],
        ...
        super().__init__()
        if weights is None: weights = [1 for i in self.grp_idxs]
        if temperatures is None: temperatures = [1 for i in self.grp_idxs]
        self.split_targets = split_targets 
        assert len(grp_idxs) == len(loss_fns) == len(weights) == len(temperatures)
        self.loss_fns = {}
        self.grp_idxs = grp_idxs
        self.weights = weights
        self.temperatures = temperatures
        for i, _loss in enumerate(loss_fns):
            _class, _args = unpack_argument(_loss, default_out={})
            self.loss_fns[i] = _class(**_args)
        
    def __call__(self, pred: torch.Tensor, targets: Union[tuple[torch.Tensor], torch.Tensor]):
        if self.split_targets:
            targets = [targets[:, self.grp_idxs[i]] for i in self.loss_fns.keys()]

        _loss = torch.tensor(0, device=pred.device, dtype=pred.dtype)
        for i in self.loss_fns.keys():
            _pred = pred[:, self.grp_idxs[i]]
            _pred = _pred / self.temperatures[i]
            _target = targets[i]
            _loss += self.loss_fns[i](_pred, _target) * self.weights[i]
        return _loss