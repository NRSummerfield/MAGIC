"""
_0 = Original
_1 = Fixed an issue with the optimizer not collecting all names in the sequence
_2 = Implemented a 2 channel "Mask_Encoder_X" so that we don't have two entire encoders soley for masks, reducing optimization complexity 
_3 = Allowing for partial trainings. Layer 1 > Layer 1+2 > Layer 1+2+3
_4 = [REDACTED] Reduced the encoders down from 1 per layered to 1 shared across all layers
_5 = Updated the saving and loading schemes to make them more efficient
_6 = Added in a .detach() option into the training and loading aspectes
_7 = Added in a "forest" option in the optimization connections
_8 = Added in different methods of handling the outputs of the mask layers for input into the next level
_9 = Added in a more flexible loading of specific layers to include reconnections of optimizers and multiple levels loaded 
_10 = Added in an optimization strategy to have one optimizer per level.
_11 = Added in a "LookForward" option for the optimizer
THISONE = Added in an option to enable a mask input at layer 0
"""
