import datetime
from .DicomImageHelper import get_contours_coords
...
def create_roi_contour(roi_data: ROIData, series_data, contour_resolution = None) -> Dataset:
    roi_contour = Dataset()
    roi_contour.ROIDisplayColor = roi_data.color
    roi_contour.ContourSequence = create_contour_sequence(roi_data, series_data, contour_resolution)
    roi_contour.ReferencedROINumber = str(roi_data.number)
    return roi_contour


def create_contour_sequence(roi_data: ROIData, series_data, contour_resolution = None) -> Sequence:
    """
    ...
    contours_coords = get_contours_coords(roi_data, series_data, contour_resolution)
